...
torontoData.ts.26week<-aggregate(torontoData.ts.week,nfrequency=1)
...
lines(ma(torontoData.ts[,3],order=168),col="blue",lwd=1)
plot(torontoData.ts[,3],col="gray",main="1 Month Moving Average")
lines(ma(torontoData.ts[,3],order=731),col="green",lwd=1)
#par(mfrow = c(2,2))
...
rwfBenchmark<-rwf(training.ts,h=length(testing.ts),drift=TRUE)
...
forecastedSES<-ses(training.ts,initial="optimal",h=length(testing.ts))
accuracy(forecastedSES,c(testing.ts))
plot(forecastedSES)

forecastedHolt<-holt(training.ts,initial="optimal", h=length(testing.ts))
accuracy(forecastedHolt, c(testing.ts))
plot(forecastedHolt)

forecastedHW<-hw(training.ts, initial = "optimal", h=length(testing.ts))
